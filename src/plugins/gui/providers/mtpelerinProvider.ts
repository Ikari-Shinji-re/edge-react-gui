...
const BUY_ONLY_PLUGINIDS: { [pluginId: string]: boolean } = {
    ...
    const out: FiatProvider = {
      ...
      getSupportedAssets: async ({ direction, paymentTypes, regionCode }): Promise<FiatProviderAssetMap> => {
        ...
        for (const token of Object.values(tokenList)) {
          ...
          if (BUY_ONLY_PLUGINIDS[pluginId] && direction === 'sell') continue
      ...
      getQuote: async (params: FiatProviderGetQuoteParams): Promise<FiatProviderQuote> => {
        ...
        if (BUY_ONLY_PLUGINIDS[pluginId] && direction === 'sell') throw new FiatProviderError({ providerId, errorType: 'assetUnsupported' })
